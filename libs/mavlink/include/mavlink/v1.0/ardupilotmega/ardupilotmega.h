...
typedef enum MAV_CMD
{
	...
	MAV_CMD_DO_SET_COLOR=253, /* Set on-board led color by using RGB. |diffenent int number represent different RGB color, eg. 1.white 2.blue 3.green 4.red 0.default color (maybe blue).| Empty| Empty| Empty| Empty| Empty| Empty|  */
	MAV_CMD_TAKEOFF_OR_LAND=254, /* Command to takeoff or land down the UAV. |true or 1: we want to takeoff the vehicle, false or 0: we want to landdown the vehicle.| Empty| Empty| Empty| Empty| Empty| Empty|  */
	MAV_CMD_CHANGE_SPEED=257, /* User can changed the UAV speed by this command. |30:slow speed mode, 60:middle speed mode, 100:fast speed mode.| Empty| Empty| Empty| Empty| Empty| Empty|  */
	MAV_CMD_TRIM=258, /* When the virtual RC in the center, the UAV cannot hover, use this command to trim. |give the channel offset amount by the trim bar.| Empty| Empty| Empty| Empty| Empty| Empty|  */
	MAV_CMD_SPINNING=260, /* Let the UAV spin in different direction. |0:keep heading, 1:clockwise, 2:counterclockwise.| Empty| Empty| Empty| Empty| Empty| Empty|  */
	MAV_CMD_THROW_FLY=261, /* Let the UAV ready to throw for auto fly. |0:normal, 1:ready to throw for fly.| Empty| Empty| Empty| Empty| Empty| Empty|  */
...
typedef enum GOPRO_CAPTURE_MODE
{
	...
	GOPRO_CAPTURE_MODE_UNKNOWN=265, /* Mode not yet known | */
	GOPRO_CAPTURE_MODE_ENUM_END=266, /*  | */
} GOPRO_CAPTURE_MODE;
#endif
